...
class CasperConfiguration extends EventManager\AbstractListenerAggregate implements ServiceManager\ServiceLocatorAwareInterface
{
  ...
  public function EventHandler (Console\Event $pEvent)
  {
    ...
    {
      $this->mConsole->writeLine('[ERROR] failed to generate casper configuration');
    }
  }
  ...
  private function WriteFile(ViewModel $pView, Console\Event $pEvent)
  {
    ...
    if (file_exists($casperFile) && false === $this->mForce)
    {
      ...
      {
        $this->mConsole->writeLine('[INFO] Casper configuration file already exists. Use --force to overwrite');
      }
    }
    else
    {
      if (false === is_writable($casperFilePath))
      {
        ...
        $this->mConsole->writeLine('[ERROR] Casper configuration file path not writable');
      }
      ...
      if ($pointer === false)
      {
        ...
        $this->mConsole->writeLine('[ERROR] Could not open casper configuration file for writing');
      }
      else
      {
        /* write the file out */
        ...
        if($this->mVerbose === true)
        {
          $this->mConsole->writeLine('[INFO] Created casper configuration');
        }
      }
    }
  }
